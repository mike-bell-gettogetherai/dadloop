...
def test_compare_counts_preselected_skills_as_in_play():
    from bench.report import compare
    base = [_row("a", 0, llm_calls=3, llm_ms=1, total_ms=1, tokens_in=1, tokens_out=1, cost=0.0, loaded=["hosting", "grilling"])]
    other = [dict(_row("a", 0, llm_calls=1, llm_ms=1, total_ms=1, tokens_in=1, tokens_out=1, cost=0.0, loaded=[]),
                  preselected=["hosting", "grilling"], load_skill_calls=0)]
    c = compare(base, other)
    assert c["per_case"]["a"]["skill_jaccard"] == 1.0, "pre-loaded bodies are in play even with zero load_skill calls"
